...
class Room(object):
    ...
    def randEmptyCoord(self, map):
        ...
        c = self.randCoord()
        while not(map.get(c) in Map.listground) or c == self.center():
            c = self.randCoord()
        return c
...
class Map(object):
    ...
    listground=[".",",","`","'"]  # A walkable ground cell
    ...
    def addRoom(self, room):
        ...
        for y in range(room.c1.y, room.c2.y + 1):
            for x in range(room.c1.x, room.c2.x + 1):
                self._mat[y][x] = random.choice(Map.listground)
    ...
    def put(self, c, o):
        ...
        self.checkCoord(c)
        self.checkElement(o)
        if not(self._mat[c.y][c.x] in Map.listground):
    ...
    def rm(self, c):
        ...
        self._mat[c.y][c.x] = random.choice(Map.listground)
    ...
    def move(self, e, way):
        ...
        if e.action==0:    
           if dest in self:
               if self.get(dest) in Map.listground:
                   self._mat[orig.y][orig.x] = random.choice(Map.listground)
                   self._mat[dest.y][dest.x] = e
                   self._elem[e] = dest
               elif (isinstance(self.get(dest), Utilise)):
...
def teleport(creature, unique):
    ...
    c = r.randCoord()
    while not theGame().floor.get(c) in Map.list:
        c = r.randCoord()
...
def jet(unique):
   ...
   while(((Devant+Creature.facing) in theGame().floor) and theGame().floor.get(Devant+Creature.facing)in theGame().floor.listground and i!=0):
      Devant+=Creature.facing
      i-=1

   
   if ((Devant+Creature.facing) in theGame().floor) and isinstance(theGame().floor.get(Devant+Creature.facing),Creature):
      theGame().floor.get(Devant+Creature.facing).action+=8
      
      
   elif ((Devant+Creature.facing) in theGame().floor) and theGame().floor.get(Devant+Creature.facing)in theGame().floor.listground:
         theGame().floor.put(Devant+Creature.facing, Utilise("used chewing-gum"))

   elif ((Devant+Creature.facing) in theGame().floor) and isinstance(theGame().floor.get(Devant+Creature.facing), Equipment) \
         or (Devant+Creature.facing) in theGame().floor and theGame().floor.get(Devant+Creature.facing)== theGame().floor.empty:
      
      theGame().floor.put(Devant,Utilise("used chewing-gum"))
...
def tir(unique):
   ...
   if ((Devant+Creature.facing) in theGame().floor) and isinstance(theGame().floor.get(Devant+Creature.facing),Creature):
      #theGame().floor.get(Devant+Creature.facing).meet(hero)
      if theGame().floor.get(Devant+Creature.facing).meet(hero) == True:
         theGame().floor.rm(Devant+Creature.facing)
...
class Game(object):
    ...
    equipments = {0: [Equipment("potion", "!", usage=lambda self, hero: heal(hero)), \
                      Equipment("gold", "o"), \
                      Equipment("gum","g", usage=lambda self, hero: jet(True))], \
                  1: [Equipment("potion", "!", usage=lambda self, hero: teleport(hero, True))], \
                  2: [Equipment("bow", usage=lambda self, hero: tir(False))], \
