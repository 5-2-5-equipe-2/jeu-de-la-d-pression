...
message=Label(fen, text='Au joueur 1 de jouer (X)')
message.grid(row = 2, column = 0, columnspan=2, padx=3, pady=3, sticky = W+E)

texte_aide = Label(fen,text= "Règle : Pour gagner, vous devez mettre votre marque X ou O sur une ligne ou sur un diagonale ou sur une colonne complète. joueur 1 à X et joueur 2 à O.")
texte_aide.grid(row=0, column=0, padx= 3, sticky = W+E)

how_play = Label (fen, text= " Cliquer sur la case de votre choix.")
how_play.grid(row=1, column=0, padx= 3, sticky = W+E)

##-----Création des boutons-----##
bouton_quitter = Button(fen, text='Quitter', command=fen.destroy)
bouton_quitter.grid(row = 4, column = 2, padx=3, pady=3, sticky = S+W+E)

bouton_reload = Button(fen, text='Rejouer', command=rejouer)
bouton_reload.grid(row = 4, column = 0, padx=3, pady=3, sticky = S+W+E)

bouton_seul = Button(fen, text='Jouer seul', command=seul)
bouton_seul.grid(row = 4, column = 1, padx=3, pady=3, sticky = S+W+E)    #ne fonctionne pas encore!!!

##-----Création du canevas-----##
dessin=Canvas(fen, bg="white", width=300, height=300)
dessin.grid(row = 3, column = 0, columnspan = 2, padx=5, pady=5)
...
class Equipment(Element):
    ...
    def __init__(self,name,abbvr=None,usage=None,transparent=False):
    ...
    '''def bourse(self):
        if self.abbrv=='o':
            self.bourse+=1
            self.use(self,creature)
            theGame().addMessage("La bourse est à"+self.bourse)'''

class Pillules(Element):  #or
    def __init__(self,name,abbvr=None,usage=None,transparent=False,bourse=1):
        Equipment.__init__(self,name,abbvr,usage,transparent)
        self.bourse=bourse

    '''def pillule_1(self):
        if self.name=="or=1":
            theGame().hero.bourse +=1

    def pillule_2(self):
        if self.name=="or=2":
            theGame().hero.bourse +=2

    def pillule_5(self):
        if self.name=="or=5":
            theGame().hero.bourse +=5

    def pillule_10(self):
        if self.name=="or=10":
            theGame().hero.bourse +=10'''
...
class Map(object):
    ...
    empty=" "
    def __init__(self,size=10,hero=None,nbrooms=10) -> None:
...
class Game(object):
    ...
    equipments = { 0: [ Equipment("potion","!",usage=lambda creature : heal(creature)), Equipment("gold","o") ,Pillules("or=1","b")],
