...
class Seller(NPC):
    ...
    def poches_fenetre(self):   #ouvre une autre fenetre tkinter
        ...
        parole1 = Label(fen,text=(self.name+": "+self.actif[0]+self.actif[1]) )
        parole1.grid(row=0, column=0, padx= 3, pady=3,columnspan=3,sticky=W)
        parole2 = Label(fen,text=("                   "+self.actif[2]+self.actif[3]))
        parole2.grid(row=1, column=0, padx= 3, pady=3, columnspan=3, sticky=W)
        objet1=Canvas(fen, bg="red", width=200, height=200)
        objet1.grid(row = 2, column = 0, padx=2, pady=2)
        '''print(self.chariot[0].abbrv)
        picture1 = theGame().dicimages.get(self.chariot[0].abbrv)
        print(picture1)
        objet1.create_image(0,0,image = picture1)'''
        ...
        objet2.grid(row =2, column = 1, padx=2, pady=2)
        '''picture1 = theGame().dicimages.get(self.chariot[0].abbrv)
        objet1.create_image(50,0,image = picture1)'''
        ...
        objet3.grid(row = 2, column = 2, padx=2, pady=2)
        '''picture1 = theGame().dicimages.get(self.chariot[0].abbrv)
        objet1.create_image(100,0,image = picture1)'''
        ...
        bouton_quitter.grid(row=3, column=3)
        fen.mainloop()
...
class Room(object):
    ...
    def randEmptyCoord2(self,map) -> Coord:
        ...
        coord=self.center()
        cc=self.center()
        while not(coord+Coord(0,-1) in self) or coord in map._elem.values() or coord==cc or coord+Coord(0,-1) in map._elem.values() or coord+Coord(0,-1)==cc:
            coord=self.randCoord()
        return coord
...
class Map(object):
    ...
    empty=" "
    def __init__(self,size=10,hero=None,nbrooms=10 ,menage=True):
    ...
    def attackpoison(self,coord:Coord) -> None:
        self.putattack(coord,Attack("Poison","psn",0,3,[Status("Poison",-1,5,prb=0.5)]))
        print("poison")
...
class Game(object):
    ...
    _actions={"z" : lambda hero : theGame().floor.move(hero,Coord(0,-1)),
              ...
              "u" : lambda hero : theGame().select(hero._inventory),
    ...
    equipments = { 0: [ Weapon("épée",3,37,"s",f=True),Equipment("gum","g", usage=lambda creature: jet(True),used=Used("used chewing-gum","u")) ,Equipment("potion","!",usage=lambda creature : heal(creature),f=True), Pills("or1","b",valeur_pillule=1), Equipment("sucette baveuse","s1",usage=lambda creature : eat(creature,1),f=True)],
                   ...
                   2: [ Armor("plaid",5,376506), Pills("or5","p",valeur_pillule=5),Equipment("sucette","s3",usage=lambda creature : eat(creature,7),f=True), Equipment("cookie choco","cc",usage=lambda creature : eat(creature,8)),Equipment("happy pills","HP",usage=lambda creature : eat(creature,10,"HP"))],
    ...
    marchande =  { 0: [ Weapon("épée",3,37,"s",f=True),Equipment("gum","g", usage=lambda creature: jet(True),used=Used("used chewing-gum","u")) ,Equipment("potion","!",usage=lambda creature : heal(creature),f=True)],
                   1: [ Equipment("lance-pierre","a", usage=lambda creature: tir(False)),Equipment("potion","!",usage= lambda creature : teleport(creature,True),f=True), Equipment("cookie choco","cc",usage=lambda creature : eat(creature,8))],
    ...
    def initgraph(self)-> None:
        ...
        img_psn1=PhotoImage(file=imgPATH+"poison1.png")
        img_psn2=PhotoImage(file=imgPATH+"poison2.png")
        img_psn3=PhotoImage(file=imgPATH+"poison3.png")
        ...
        self.dicimages={'magic':angry3,'showcase': hero_fi.zoom(3),"." : sol_img1,"," : sol_img2,"`" : sol_img3,"´" : sol_img4,".m" :wetsol_img1 , ",m" : wetsol_img2, "`m" : wetsol_img3,"´m" : wetsol_img4, "@" : [hero_fi,hero_bi,hero_li,hero_ri],"@*": [hero_tf,hero_tb,hero_tl,hero_tr],"!" : pot_img3,"G" : ted_img,"W":ted_img,"O":sad_img,"B":ted_img,"D":ted_img, "fv":ghost_img ,"s":bequille_img, "a" : img_stonelance, "!":pot_img1, "c":pot_img3, "b":or1_img, "j":or2_img, "p":or5_img, "P":or10_img ,"M":marchand_f ,'inventory':hotbar , 'faim100' : faim100 , 'faim75' : faim75 , 'faim50' : faim50 , 'faim25' : faim25 , 'faim0': faim0, 'empty' : vide , 'health' : vie,'dialogue' : dialoguebox.zoom(5), ">" : esc_up, "<" : esc_down ,"u":chew_img ,"g":gum_img ,"Be1":bedup ,"Be2":beddown ,"Fa":wheelchair ,"Po":flowerpot ,"s1":sucette1_img , "s2":sucette2_img , "s3":sucette3_img , 'herobox0':herobox0 , 'herobox10':herobox10 , 'herobox20':herobox20, 'herobox30':herobox30, 'herobox40':herobox40 ,'herobox50':herobox50 ,'herobox60':herobox60 ,'herobox70':herobox70 ,'herobox80':herobox80 ,'herobox90':herobox90 , 'cc':cookie_img, "HP":happypills_img}
        ...
        self.dicatk={"psn":[img_psn1,img_psn2,img_psn3]}
